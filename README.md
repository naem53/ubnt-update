...
2. Клонируем репозиторий, и переходим в склонированный каталог.
```bash
git clone https://github.com/naem53/ubnt-update.git
cd ubnt-update
```
3. Запускаем стрим командой:
```bash
sudo docker compose up -d --build
```
4. Добавляем DNS записи в Unifi, для доменов, которые будут заворачиваться в туннель, как написано [тут](#добавление-dns-записей-в-unifi).
> ⚠️ **Примечание:**  
> В поле **IP Address** указываем **IP сервера** в туннеле
5. Добавляем статический маршрут до обратной стороны туннеля, как написано [тут](#добавление-статического-маршрута).
6. Проверяем работу, как написано [тут](#проверка-работы).
